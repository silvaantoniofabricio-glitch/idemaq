...
import { Card, Badge, EmptyState, DeltaPill } from '../../components/ui'
...
function SecHeader({ T, dark, icon, cor, children }) {
  return (
    <div style={{
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ display: 'grid', gap: 12, gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))' }}>
        <KPI T={T} dark={dark} label="Receita bruta"    valor={fmtBRL(data.receitas)}      cor={azul}     delta={data.delta.receitas}  icon="ti-arrow-down-circle" />
        <KPI T={T} dark={dark} label="Receita líquida"  valor={fmtBRL(data.receitaLiquida)} cor={azulClaro} icon="ti-cash"              detalhe={`(− ${fmtBRL(data.totalTaxasMaq)} em taxas)`} />
        <KPI T={T} dark={dark} label="Despesas pagas"   valor={fmtBRL(data.despesas)}       cor={amarelo}  delta={data.delta.despesas}  icon="ti-arrow-up-circle"  deltaInverso />
        <KPI T={T} dark={dark} label="Resultado"        valor={fmtBRL(data.lucro)}          cor={data.lucro >= 0 ? azul : vermelho} delta={data.delta.lucro} icon="ti-trending-up" />
        <KPI T={T} dark={dark} label="Margem"           valor={`${data.margem}%`}           cor={corHero(dark)} icon="ti-percentage" />
        <KPI T={T} dark={dark} label="A receber"        valor={fmtBRL(data.aReceber)}       cor={azulClaro} icon="ti-clock"            detalhe={data.aPagar > 0 ? `A pagar: ${fmtBRL(data.aPagar)}` : null} />
      </div>

      {/* Receitas detalhadas */}
      <Card T={T} dark={dark}>
        <SecHeader T={T} dark={dark} icon="ti-arrow-down-circle" cor={azul}>
          Receitas detalhadas
        </SecHeader>
      ...
      {/* Despesas detalhadas */}
      <Card T={T} dark={dark}>
        <SecHeader T={T} dark={dark} icon="ti-arrow-up-circle" cor={amarelo}>
          Despesas detalhadas
        </SecHeader>
        <div style={{ display: 'grid', gap: 16, gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))' }}>
          <TabelaCategorias T={T} dark={dark} titulo="Por categoria"
      ...
      {/* Top 10 despesas */}
      <Card T={T} dark={dark}>
        <SecHeader T={T} dark={dark} icon="ti-list-numbers" cor={amarelo}>
      ...
      <div style={{ display: 'grid', gap: 12, gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))' }}>
        <Card T={T} dark={dark}>
          <SecHeader T={T} dark={dark} icon="ti-clock" cor={azulClaro}>
            A receber no período ({fmtBRL(data.aReceber)})
          </SecHeader>
          {data.aReceberLista.length === 0 ? (
            <EmptyState T={T} compact icon="ti-check" title="Nada em aberto"
        ...
        <Card T={T} dark={dark}>
          <SecHeader T={T} dark={dark} icon="ti-clock" cor={amarelo}>
      ...
      {/* Fluxo diário */}
      <Card T={T} dark={dark}>
        <SecHeader T={T} dark={dark} icon="ti-chart-bar" cor={azul}>
...
function KPI({ T, dark, label, valor, delta, cor, icon, detalhe, deltaInverso }) {
  return (
    <Card T={T} dark={dark}>
      ...
      <div style={{ fontSize: 22, fontWeight: 800, color: cor, fontVariantNumeric: 'tabular-nums', letterSpacing: '-0.02em' }}>
        {valor}
      </div>
      {detalhe && <div style={{ fontSize: 11, color: T.textMuted, marginTop: 4 }}>{detalhe}</div>}
    </Card>
  )
}

// ─── TabelaCategorias (sub-seção dentro de Card) ─────────────────────────────
function TabelaCategorias({ T, dark, titulo, itens, total, cor }) {
  return (
    <div style={{
      background: T.cardAlt, borderRadius: 6, border: `1px solid ${T.border}`, padding: '10px 12px',
    }}>
      <div style={{ fontSize: 11, fontWeight: 700, color: T.textMuted, marginBottom: 10, textTransform: 'uppercase', letterSpacing: '.04em' }}>
        {titulo}
      </div>
      {itens.length === 0 ? (
        <div style={{ fontSize: 12, color: T.textMuted, padding: '4px 0' }}>—</div>
      ) : (
            ...
            return (
              <div key={i.label}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 4 }}>
                  ...
                  <span style={{ color: corHero(dark), fontWeight: 600, fontVariantNumeric: 'tabular-nums' }}>
                    {fmtBRL(i.valor)}
                    <span style={{ color: T.textMuted, fontWeight: 400, marginLeft: 6 }}>· {pct}%</span>
                  </span>
                </div>
                <div style={{ width: '100%', height: 5, borderRadius: 3, background: dark ? 'rgba(255,255,255,0.08)' : '#e4e7eb', overflow: 'hidden' }}>
...
function ListaDespesas({ T, dark, itens }) {
  if (itens.length === 0) return <EmptyState T={T} compact icon="ti-list" title="Sem despesas no período" description="" />
  return (
    <div style={{ display: 'flex', flexDirection: 'column' }}>
      {itens.map((d, i) => (
        <div key={d.id} style={{
          ...
          <div style={{ fontSize: 12, fontWeight: 700, color: T.textMuted, textAlign: 'center' }}>#{i + 1}</div>
          <div style={{ minWidth: 0 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: corHero(dark), whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
          ...
          <div style={{ fontSize: 13, fontWeight: 700, color: corEtapa('yellow', dark), textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>
            {fmtBRL(d.valor)}
          </div>
      ...
      {itens.map((it, i) => (
        <div key={it.id} style={{
          ...
          <div style={{ minWidth: 0 }}>
            ...
            <div style={{ fontSize: 11, color: T.textMuted, marginTop: 2 }}>{it.categoria}</div>
          </div>
          <div style={{ fontSize: 13, fontWeight: 700, color: cor, textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>
            {fmtBRL(it.valor)}
          </div>
...
function FluxoDiario({ T, dark, dados }) {
  if (!dados || dados.length === 0) return <EmptyState T={T} compact icon="ti-chart-bar" title="Sem movimentação" description="" />
  const azul   = corEtapa('blue', dark)
  const amarelo = corEtapa('yellow', dark)
  ...
  return (
    <div style={{ overflowX: 'auto' }}>
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, padding: '8px 4px 4px', minHeight: 160 }}>
        {dados.map(d => {
          ...
          return (
            <div key={d.dia}
              ...
              <div style={{ display: 'flex', alignItems: 'flex-end', gap: 2, height: 130 }}>
                <div style={{ width: 12, height: hEntrada, background: azul,   borderRadius: '3px 3px 0 0' }} />
                <div style={{ width: 12, height: hSaida,   background: amarelo, borderRadius: '3px 3px 0 0' }} />
              </div>
              <div style={{ fontSize: 10, color: T.textMuted }}>{fmtDataDDMM(d.dia)}</div>
            </div>
          )
        })}
      </div>
      <div style={{ display: 'flex', gap: 16, justifyContent: 'center', fontSize: 11, color: T.textMuted, marginTop: 8 }}>
        <span><span style={{ display: 'inline-block', width: 10, height: 10, background: azul,   borderRadius: 2, marginRight: 4 }} />Entradas</span>
        <span><span style={{ display: 'inline-block', width: 10, height: 10, background: amarelo, borderRadius: 2, marginRight: 4 }} />Saídas</span>
      </div>
    </div>
  )
}
  ...
  return (
    <Card T={T} dark={dark} accent={vermelho}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, color: vermelho }}>
        ...
        <span style={{ fontSize: 13, fontWeight: 600 }}>Erro ao carregar relatório</span>
      </div>
