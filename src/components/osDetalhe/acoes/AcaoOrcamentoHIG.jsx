...
function GrupoItens({ tipo, itens, subtotal, T, dark, onAdd, onRemove, adicionando }) {
  const count = itens.length
  const addBtn = !adicionando && (
    <button type="button" onClick={() => onAdd(tipo.id)} style={{
      ...higType('footnote'),
      ...
      <TI name="plus" size={13} color={HIG_COLOR.tintIdemaq} />
      Adicionar
    </button>
  )
      ...
      >
        {itens.length > 0 ? (
            ...
            <React.Fragment key={it.id || i}>
              ...
              <div style={{
                ...
                <span style={{
                  ...
                  {fmtBRL((it.qtd || 1) * (it.valor_unitario || 0))}
                </span>
                <button type="button" onClick={() => onRemove(it.id)} style={{
          ...
          <div style={{
            ...
            <TI name="plus-circle" size={16} color={T.textDim} />
            <span style={{ ...higType('subheadline'), color: T.textMuted, fontStyle: 'italic' }}>
              Nenhum item — toque em Adicionar
            </span>
          </div>
        )}
      </HIGSection>
...
export default function AcaoOrcamentoHIG({ os, onUpdateOS, onMoverOS }) {
  ...
  // Constroí props passadas para GrupoItens com save/cancel embutidos
  function onAddWrapper(tipoId) {
    const fn = (dados) => handleSaveItem(dados)
    fn.__save   = handleSaveItem
    fn.__cancel = () => setAdicionandoTipo(null)
    setAdicionandoTipo(tipoId)
    return fn
  }
  ...
  return (
    <div style={{
      ...
      {/* 2. Grupos de itens */}
      {TIPOS.map(tipo => {
        const adicionando = adicionandoTipo === tipo.id
        const addFn = (id) => setAdicionandoTipo(id)
        addFn.__save   = handleSaveItem
        addFn.__cancel = () => setAdicionandoTipo(null)
        return (
          <React.Fragment key={tipo.id}>
            ...
            {adicionando && (
              <AddItemForm
                tipo={tipo} T={T} dark={dark} saving={saving}
