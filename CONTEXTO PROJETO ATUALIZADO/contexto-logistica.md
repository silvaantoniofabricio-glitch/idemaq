...
- **Slots Rota A/B/C**: `dataAtiva = HOJE` fixo. Auto-cria 3 rotas ao abrir o dia via guard `useRef(criandoRotasRef)`. `sql/17-rota-nome.sql` adiciona coluna `nome` + novo UNIQUE — **ainda não aplicado** (Toni roda quando precisar dos slots automáticos).
