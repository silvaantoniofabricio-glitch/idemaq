...
- ✅ **Botão Garantia no histórico de OS do cliente** (03/06/2026): em `ClienteDetalheModal.HistoricoOS`, cada OS `concluido` do tipo `atendimento` que **não seja garantia** e tenha menos de 90 dias desde a conclusão mostra botão verde "Garantia". Clique chama `criarOSDerivada(os.id, { tipo:'atendimento', etapa:'aguardando_agendamento', garantia:true, valor_total:0, garantia_dias:90 })` → nova OS aparece no Kanban via Realtime, na 1ª etapa (Agenda), com R$0, copiando cliente+equipamento da origem. Helper local `estaEmGarantia(os)` usa `data_conclusao || abertura + garantia_dias` (não depende de `os_historico.entrega`).
