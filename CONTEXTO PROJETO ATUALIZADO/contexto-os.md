...
- **Tabela nova**: `roteiro_item` (`sql/83-roteiro-dia.sql`) — 1 linha = 1 tarefa do dia de UM funcionário. Campos: `dia` (date local Cuiaba 'YYYY-MM-DD'), `responsavel_id`, `ordem`, `texto`, `os_id` (FK opcional → `os`), `urgente`, `feito`, `feito_em` + auditoria + soft-delete. RLS: funcionário vê/edita o próprio, dono tudo. **Gotcha**: o SQL já faz `ALTER PUBLICATION supabase_realtime ADD TABLE roteiro_item` — sem isso o "ao vivo" não funciona. **PENDENTE: Toni rodar sql/83 no Supabase** (até lá o hook retorna `tabelaAusente` e a UI mostra "rode o SQL", sem quebrar — padrão do `usePonto`).
