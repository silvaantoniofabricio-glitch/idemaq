...
1c. ⚠️ **Cadastro cliente inline OK · equipamento BLOQUEADO por schema** (20/05/2026, Onda 5 + 2 hotfixes). `FormClienteEdit` e `FormEquipamentoEdit` (`src/components/osDetalhe/`) foram criados. Cliente: SELECT fresco por `id` na abertura (useOS só traz nome+telefone) + parser do endereço gravado por `criarClientePersist` + UPDATE em `cliente` + `onSalvarOk` dispara `osRefetch` propagado Kanban→OSDetalhe→Header → ✅ funcionando. Equipamento: **as colunas `marca_equipamento/modelo_equipamento/defeito_relatado/numero_serie` NÃO existem na tabela `os` em prod** — confirmado por probe (PostgREST 42703 no SELECT, PGRST204 no INSERT). O hotfix que adicionou ao SELECT do useOS quebrou o Kanban inteiro (osList vazio) e expôs que o INSERT do NovaOSModal também já estava silenciosamente falhando há tempos. **Hotfix 2** reverteu: useOS volta a hardcodar `''` nesses campos · `osPatch.UI_TO_DB` continua mapeando mas `COLUNAS_SAFE` removeu as 3 chaves (vão pra `skipped[]`, in-memory) · NovaOSModal no _legacy parou de mandar essas colunas no payload (exceção cirúrgica autorizada — mesmo precedente do trecho CLIENTES_MOCK). **Pendência destravante**: aplicar `sql/10-os-equipamento.sql` no Supabase SQL Editor pra criar as 4 colunas + reativar os 3 lugares marcados no header do SQL (`_legacy` payload, `osPatch.COLUNAS_SAFE`, `useOS` SELECT+map). Sem isso, FormEquipamentoEdit roda mas o salvar não persiste (UI só optimistic).
