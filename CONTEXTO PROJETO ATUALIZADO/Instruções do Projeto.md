...
- **Kanban + Nova OS + OSDetalhe** com UI completa (drag-and-drop, modal 3-abas, 10 Ações por etapa)
- **Ações por etapa implementadas — TODAS as 10 (Módulo 03 visual completo, 17-18/05/2026)**:
  - **Recebido (Pré-diagnóstico)**: 4 testes × OK/Defeito/Barulho + obs
...
- **Atalhos no Desktop (7 terminais dedicados)**: `Painel Idemaq` · `OS Idemaq` · `Clientes Idemaq` · `Logistica Idemaq` · `Estoque Idemaq` · `Financeiro Idemaq` · `Relatorios Idemaq`. Cada um abre Windows Terminal na pasta do projeto, seta `$env:IDEMAQ_TERMINAL` com a área, mostra banner ciano com o foco e roda `claude` automaticamente. Toni trabalha múltiplas features em paralelo, uma por terminal.

### 🚧 Em andamento / Próximo passo
- **Módulo 00c — Migração Mock → Supabase** — bloco crítico que destrava 01/02/03 (próximo a fazer)

### 📅 Roadmap (ordem prevista)
1. ✅ Kanban de OS — UI pronta, falta save real (Módulo 01)
2. ✅ Formulário Nova OS — UI pronta, falta save real (Módulo 02)
3. 🚧 Fluxo completo da OS — Diagnóstico+Orçamento+Oficina+Pagamento prontos, faltam Pré-diag/Teste/Entrega/Conclusão (Módulo 03)
4. 🟡 Clientes — mock visual pronto, falta conectar
5. 🟡 Logística — mock visual pronto, falta Google Maps + tabela `rota`
6. 🟡 Estoque — mock visual + modais de detalhe (Peça/Máquina) prontos, falta conectar
7. 🟡 Financeiro completo — mock visual reformulado Bling-style (filtros/tabela/bulk), falta schema parte 2
8. 🟡 Relatórios com IA — 6 relatórios mock prontos, falta queries reais + Claude API
9. 📅 Configurações
10. 📅 Automações n8n + Z-API
11. 📅 Agente de reativação de clientes

**Crítico antes de tudo**: Módulo 00c (migração Mock → Supabase). Sem isso as telas acima ficam só "demonstração".
